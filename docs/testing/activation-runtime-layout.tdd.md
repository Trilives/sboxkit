...
/var/lib/sboxkit/revisions/<revision>/
/var/lib/sboxkit/current -> revisions/<revision>
...
- As an operator, I want the systemd service to start only the current
  revision so service rollouts are explicit and inspectable.
...
- As a packager, I want the built-in WebUI installed under `/usr/share/sboxkit/ui`
  and referenced from generated runtime configs when state UI files are absent.
...
```text
/usr/lib/sboxkit/sing-box run -c /var/lib/sboxkit/current/config.json
```
...
|---|---|---|---|
| 1 | Default root paths expose state, revisions, current symlink, cache DB, cache downloads, and admin config | `internal/paths/paths_test.go` | PASS |
| 2 | Rendered systemd unit starts `/usr/lib/sboxkit/sing-box` with `current/config.json` | `internal/system/service_test.go` | PASS |
| 3 | Service sync stages config/manifest into a revision and switches `current` | `internal/system/service_test.go` | PASS |
