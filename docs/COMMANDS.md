...
| `sboxkit init` | 初始化（首次部署）：先选语言，再走下载代理→TUN→局域网→添加首个订阅（clash / sing-box 原生 / base64 / 本地文件四选一）→注册服务→可选更新内核/geo 数据→网络自愈→更新定时器。各步骤各自独立可 ^R 回退，互不连带——例如服务已注册启动后，后续可选步骤失败不会把服务卸载回退。交互式主菜单检测到服务未注册时会询问是否触发，此子命令主要供脚本/无人值守场景显式调用 |
