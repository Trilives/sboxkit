...
| 定制层 | 拆成「部署设置」与「分流增强」两个分组：TUN / 局域网代理 / 面板对外监听 / 下载代理 / GitHub 镜像与 Token / 默认出站（节点切换目标分组）等 |
...
| systemd 集成 | 主服务 + 网络自愈 watchdog + 每周更新定时器，统一暂停/启动；Web 面板走 sing-box 内置的 `:9090/ui/` 路径，不再单独占用端口 |
...
| 中英双语 | 默认英文启动（部分终端无法正常显示中文），初始化第一步与主菜单「Language / 语言」均可切中文，持久化到 `customize.json`；`SBOXKIT_LANG=en\|zh` 可覆盖 |
